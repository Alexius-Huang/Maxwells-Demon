...
class FileSystem extends React.Component {
  ...
  recursiveRenderFolderStructure(structure, index = 0) {
    return (
      <div className="virtual-folder" key={ index } data-expand={ structure.expand } data-folder-id={ structure.id }>
        <p><span className={ `fa fa-folder${ structure.expand ? '-open' : '' }` }></span> { structure.folderName }</p>
