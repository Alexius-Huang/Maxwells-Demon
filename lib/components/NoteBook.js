...
class NoteBook extends React.Component {
  ...
  render() {
    ...
    const renderCells = cells.map(({ id, type, content, partial, dependentCellID, dependent }, index) => 
      <Cell key={ id } index={ index } cellId={ id } type={ type } partial={ partial } dependentCellID={ dependentCellID } dependent={ dependent }>{ content }</Cell>)
